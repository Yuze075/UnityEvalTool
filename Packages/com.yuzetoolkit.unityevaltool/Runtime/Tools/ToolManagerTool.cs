...
namespace YuzeToolkit
{
    ...
    [EvalTool("UnityEval", "UnityEvalTool catalog, tool management, and authoring guidance.")]
    public sealed partial class ToolManagerTool
    {
        [UnityEngine.Scripting.Preserve]
        [EvalFunction("Return a prompt that explains how to author a valid loader-backed JavaScript tool.")]
        public string getJsToolAuthoringPrompt() => EvalToolRegistry.GetJsToolAuthoringPrompt();

        [UnityEngine.Scripting.Preserve]
        [EvalFunction("List all registered tools. C# and loader-backed JavaScript tools can be enabled or disabled.")]
        public Dictionary<string, object?> listTools(bool refresh = false) => EvalToolRegistry.GetIndex(refresh);

        [UnityEngine.Scripting.Preserve]
        [EvalFunction("Return full metadata for one tool.")]
