    ...
    public sealed class UnityEvalToolAuthorizationSettings : ScriptableObject
    {
        ...
        public void SetRequireToken(bool requireToken)
        {
            if (requireToken && !HasVerifier)
                throw new InvalidOperationException("Configure a UnityEvalTool token before enabling verification.");
        ...
        public static void ValidateToken(string token)
        {
            if (string.IsNullOrEmpty(token))
                throw new ArgumentException("UnityEvalTool token cannot be empty.", nameof(token));
            if (token.Length > MaxTokenLength)
                throw new ArgumentException($"UnityEvalTool token cannot exceed {MaxTokenLength} characters.", nameof(token));
            foreach (var character in token)
            {
                var allowed = character is >= 'A' and <= 'Z' or >= 'a' and <= 'z' or >= '0' and <= '9' or '_' or '-';
                if (!allowed)
                    throw new ArgumentException(
                        "UnityEvalTool token may contain only ASCII letters, digits, underscore, and hyphen.", nameof(token));
            }
        }
        ...
        private void ValidateStoredConfiguration()
        {
            if (!string.Equals(_algorithm, AlgorithmVersion, StringComparison.Ordinal))
                throw new InvalidOperationException($"Unsupported UnityEvalTool token algorithm '{_algorithm}'.");
            if (!HasVerifier)
                throw new InvalidOperationException("UnityEvalTool token verification is enabled without a verifier.");
            try
            {
                if (Convert.FromBase64String(_saltBase64).Length != 16 ||
                    Convert.FromBase64String(_verifierBase64).Length != 32)
                    throw new InvalidOperationException("UnityEvalTool token verifier has invalid byte lengths.");
            }
            catch (FormatException ex)
            {
                throw new InvalidOperationException("UnityEvalTool token verifier is not valid Base64.", ex);
            }
        }
