    ...
    internal static class EditorCompilationMonitor
    {
            ...
            out Dictionary<string, object?> state)
        {
            ...
            var message = shouldStopPlayMode
                ? "Unity is playing or changing play mode. The request is pending; UnityEvalTool requested PlayMode exit and will run it after Unity returns to stable EditMode."
        ...
        private static void TryRunPendingRequest()
        {
            ...
            {
                Debug.LogError($"[UnityEvalTool] Failed to run pending {kind} request: {ex.Message}");
            }
        }
